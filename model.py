...
class MODEL(object):
    def __init__(self, 
        ...
        self.model_list = ["googleLeNet_v1", "resNet_v1", "srcnn_v1", "grr_srcnn_v1", 
                           ...
                           "edsr_lsgan_lap", "edsr_lsgan_lap_v2", "edsr_lsgan_lap_att_v2", "EDSR_WGAN_att","edsr_attention_v3",
                           "EDSR_WGAN_att_vgg_v1"]
    ...
    def build_edsr_lsgan(self):###
        ...
        # Build model
        _, dis_t = mz.build_model({"d_inputs":self.target,"d_target":self.target,"scale":self.scale,"feature_size" :64,"reuse":False, "is_training":True})
        gen_f, dis_f = mz.build_model({"d_inputs":None,"d_target":self.target,"scale":self.scale,"feature_size" :64, "reuse":True, "is_training":True})
        ...
        _, d_hat = mz.build_model({"d_inputs":x_hat,"d_target":self.target,"scale":self.scale,"feature_size" :64, "reuse":True, "is_training":True})
        ...
        self.g_l1loss = tf.reduce_mean(tf.losses.absolute_difference(target,gen_f))
        self.g_loss =  -10000.0*disc_fake_loss + reconstucted_weight*self.g_l1loss
        ...
        gvs_g_dis = optimizer.compute_gradients(-10000.0*disc_fake_loss, var_list=generator_variables)
    ...
    def train_edsr_lsgan(self):
        ...
        epoch_pbar = tqdm(range(796,self.epoch))
        for ep in epoch_pbar:            
            # Run by batch images
            random.shuffle(dataset) 
            train_data, train_label  = zip(*dataset)
    ...
    def build_edsr_lsgan_up(self):
        ...
        self.d_loss =   disc_fake_loss - disc_ture_loss + d_gp
        self.g_l1loss = tf.reduce_mean(tf.losses.absolute_difference(target,gen_f))
        self.g_loss =  -10.0*disc_fake_loss + reconstucted_weight*self.g_l1loss
        
       
        train_variables = tf.trainable_variables()
        generator_variables = [v for v in train_variables if v.name.startswith("EDSR_gen")]
        discriminator_variables = [v for v in train_variables if v.name.startswith("EDSR_dis")]
        self.train_d = tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999).minimize(self.d_loss, var_list=discriminator_variables, name="d_adam")
        self.train_g = tf.train.AdamOptimizer(self.lr, beta1=0.5, beta2=0.999).minimize(self.g_loss, var_list=generator_variables, name="g_adam")
        self.train_l1 = tf.train.AdamOptimizer(self.lr,beta1=0.5, beta2=0.999).minimize(self.g_l1loss, var_list=generator_variables, name="l1_adam")
    ...
    def train_edsr_lsgan_up(self):
        ...
        epoch_pbar = tqdm(range(532,self.epoch))
        for ep in epoch_pbar:            
            # Run by batch images
            random.shuffle(dataset) 
            train_data, train_label  = zip(*dataset)
            ...
            for idx in batch_pbar:                
                ...
                if action == 0:
                    ...
                    for i in range(5):
                        ...
                        _, tsum = self.sess.run([self.train_d, self.merged_summary_train],
                                                 feed_dict={self.input: batch_images,
                                                            self.image_target: batch_labels,
                                                            self.dropout: 1.,
                                                            self.lr:learning_rate}, options=options, run_metadata=run_metadata)

                        summary_writer.add_run_metadata(run_metadata, tag='step {}_{}_{}'.format(ep*itr_per_epoch,idx,i))
                        summary_writer.add_summary(tsum, ep)
                        """
                        ...
                        #with open('timeline_02_step_{}.json'.format(1), 'w') as f:
    ...
    def build_edsr_wgan_encode(self):###
        """
        Build SRCNN model
        """        
        # Define input and label images
        ...
        # Build model
        gen_f, dis_f = mz.build_model({"d_inputs":None,"d_target":self.target,"scale":self.scale,"feature_size" :64, "reuse":None, "is_training":True})
        _, dis_t = mz.build_model({"d_inputs":self.target,"d_target":self.target,"scale":self.scale,"feature_size" :64,"reuse":True, "is_training":True})
        ...
        reconstucted_weight = 10.0  #StarGAN is 10
        self.d_loss =   disc_fake_loss - disc_ture_loss
        self.g_l1loss = tf.reduce_mean(tf.losses.absolute_difference(target,gen_f))
        self.g_loss =  -1.0*disc_fake_loss + reconstucted_weight*self.g_l1loss
    ...
    def train_edsr_wgan_encode(self):
        """
        Training process.
        """     
        print("Training...")
        ...
        epoch_pbar = tqdm(range(5082,self.epoch))
        for ep in epoch_pbar:            
            # Run by batch images
            random.shuffle(dataset) 
            train_data, train_label  = zip(*dataset)
    ...
    def build_edsr_lsgan_recursive(self):###
        """
        Build SRCNN model
        """        
        # Define input and label images
    ...
    def train_edsr_lsgan_recursive(self):
        """
        Training process.
        """     
        print("Training...")
...
#                print("Current learning rate: [{}]".format(learning_rate))
    ...
    def build_edsr_attention_v3(self):###
        ...
        ### Build model       
        gen_f, _ = mz.build_model(kwargs={'reuse':False})
        
        self.g_l1loss = tf.reduce_mean(tf.losses.absolute_difference(target,gen_f))
        

        self.train_g = tf.train.AdamOptimizer(self.lr).minimize(self.g_l1loss )
    ...
    def train_edsr_attention_v3(self):
...
#            tf.summary.image("dis_diff",tf.abs(dis_f-dis_t)*255, collections=['train'])
    ...
    def train_EDSR_WGAN_att_vgg_v1(self):
...
#                print("Current learning rate: [{}]".format(learning_rate))
    ...
    def build_edsr_attention_v3(self):###
        ...
        ### Build model       
        self.gen_f, self.att = mz.build_model(kwargs={'reuse':False})
    ...
    def train_edsr_attention_v3(self):
        ...
        # Define dataset path
        #96X96
        test_dataset = self.load_divk("/home/ubuntu/dataset/SuperResolution/att_test_img/",type="test")
        ...
        for i in range(3):
            ...
            att_layer, output  = self.sess.run([self.att, self.gen_f] ,
                                                 feed_dict={
                                                             self.input: test_data[i].reshape(1,96,96,3),  
                                                             self.image_target: test_label[i].reshape(1,96,96,3),
                                                             self.curr_batch_size: len(test_label),
                                                             self.dropout: 1.,
                                                           })
            att_layer = att_layer.reshape(128*3)    
            output = output.reshape(96,96,3)                                                                  
            print(np.argmax(att_layer), np.max(att_layer), np.shape(output))
            plt.plot(att_layer)
            plt.show()
            plt.imshow(output)
            plt.show()
